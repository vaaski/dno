...
/**
 * T-Rex runner.
 * @param {string} outerContainer Outer containing element or its selector.
...
function Runner(outerContainer, opt_config = {}) {
  ...
  this.outerContainerEl =
    outerContainer instanceof HTMLElement
      ? outerContainer
...
Runner.prototype = {
  ...
  async loadSounds() {
    if (!IS_IOS) {
      ...
      for (const sound in Runner.sounds) {
        let soundSrc = resourceTemplate.querySelector("#" + Runner.sounds[sound]).src
  ...
  init() {
    ...
    this.canvasCtx = /** @type {CanvasRenderingContext2D} */ (this.canvas.getContext(
      "2d"
    ))
  ...
  isCanvasInView() {
    return (
      this.containerEl.getBoundingClientRect().top > Runner.config.CANVAS_IN_VIEW_OFFSET
    )
  },
  ...
  onGamepadConnected(e) {
    if (!this.pollingGamepads) this.pollGamepadState()
  },
  ...
  pollActiveGamepad(gamepads) {
    ...
    if (gamepad.buttons.length >= 2) this.pollGamepadButton(gamepad, 1, 40) // Duck

    if (gamepad.buttons.length >= 10) this.pollGamepadButton(gamepad, 9, 13) // Restart
...
DistanceMeter.prototype = {
  ...
  flashHighScore() {
    ...
    if (paint) this.drawHighScore()
    else this.clearHighScoreBounds()
...
Horizon.prototype = {
  ...
  updateClouds(deltaTime, speed) {
    ...
    if (numClouds) {
      for (let i = numClouds - 1; i >= 0; i--) this.clouds[i].update(cloudSpeed)
